...
aria.GridCombobox.prototype.handleInputKeyDown = function (evt) {
  ...
  if (activeItem) {
    ...
    const selectedItem = this.getItemAt(activeRowIndex, this.selectionCol);
...
aria.GridCombobox.prototype.handleGridClick = function (evt) {
  ...
  const selectItem = row.querySelector('.result-cell');
...
aria.GridCombobox.prototype.updateResults = function () {
  ...
  let results = this.searchFn(searchString);
 ...
 */
aria.GridCombobox.prototype.getItemAt = function (/** @type {string} */ rowIndex, /** @type {string} */ colIndex) {
